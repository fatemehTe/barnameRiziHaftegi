  ...
  <div class="row">
    <div class="d-flex flex-row mb-2 justify-content-between align-items-center">
      ...
      <div class="d-flex flex-row">
        ...
        <div class="c-ffffff rounded-1 bg-blue p-3 cursor-pointer text-center ms-1" @click="$router.push('/moshaverCMExtra')"> 
        برنامه ریزی متفرقه
        </div>  
        </div>
        <div class="d-flex flex-column">
...
export default {
  data() {
    return {
      ...
      timeTable : ['ریاضی','فیزیک','شیمی','ادبیات','هندسه','گسسته'],
  ...
  methods:{
    getReshte(){
        axios.get(`/api/User/GetUserReshte/2`).then((response) => {
          ...
          axiosQuestion.get(`/Api/Lesson/GetAllByEducationTreeId/${this.reshteId}`).then((responsee) => {
            ...
            this.makeFirstColumn()
            this.makeLessons()
         }); 
        });   
      },
      makeFirstColumn(){
        axios.get(`/api/BarnameHaftegi/GetDorosByReshteId/${this.reshteId}`).then((response) => {
    ...
    taskCopy(){
      ...
      editTaskFunc(){
       ...
       if(q >= 60){
        ...
        span12.innerHTML = this.tashrihi === false ? this.testCount:this.lessonOptions.filter(x=>x.value == this.lessons)[0].label 
      },
    ...
    addTask(copy){
      if((this.lessons == 0 || this.lessons == null)){
        ...
        }else if(this.lessons != 0 && this.taskType == null){
          this.showTaskTypeErr = true
        }else if(this.lessons != 0  && this.selectedArticle == null){
  ...
  mounted(){
    this.getReshte()
    this.getDate()
    let topicList = this.topicStore.fillListt();
